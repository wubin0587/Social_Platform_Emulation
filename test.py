import sys
import os
import subprocess
from datetime import datetime

def run_command_injection_test():
    """
    通过命令行注入的方式，调用 run.py 来执行一个预设的测试场景。
    测试场景: BasicSimulation, 1SW+3SF+1ER, 100轮
    """
    print("--- 准备通过命令行注入方式运行测试 ---")
    
    # --- 1. 定义测试参数 ---
    
    # 确定要使用的Python解释器 (使用 sys.executable 可以确保与当前环境一致)
    python_executable = sys.executable
    
    # 确定 run.py 脚本的路径
    # os.path.dirname(__file__) 是 test.py 所在的目录，也就是项目根目录
    project_root = os.path.dirname(os.path.abspath(__file__))
    run_script_path = os.path.join(project_root, "run.py")
    
    # 指定要使用的配置文件路径
    # 注意：这里的路径是相对于项目根目录的，与您在命令行中输入的习惯保持一致
    config_file = "config/test.yaml" 
    
    # 为输出文件生成一个唯一的名字
    output_filename = f"test_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    
    # --- 2. 构建完整的命令行参数列表 ---
    # 每一个空格分隔的参数都应该是列表中的一个独立元素
    command = [
        python_executable,
        run_script_path,
        '--sim_type', 'basic',
        '--config', config_file,
        '--metrics', 'opinion_variance', 'homophilic_bimodality_coefficient',
        '--all_layers',
        '--output', output_filename
    ]
    
    # --- 3. 打印将要执行的命令，方便调试 ---
    print("\n将要执行的命令:")
    # 使用 subprocess.list2cmdline 来美观地显示命令
    print(subprocess.list2cmdline(command))
    print("\n--- 开始执行子进程 ---")
    
    # --- 4. 执行命令 ---
    # subprocess.run 会等待命令执行完毕
    # check=True 表示如果 run.py 报错（返回非零退出码），这里也会抛出异常
    # text=True, capture_output=True 可以捕获输出，但为了实时看到进度，我们不捕获
    try:
        result = subprocess.run(command, check=True, text=True, encoding='utf-8')
        print("\n--- 子进程执行成功 ---")
        # print("STDOUT:\n", result.stdout) # 如果需要，可以取消注释来打印 run.py 的输出
    except FileNotFoundError:
        print(f"错误: 找不到脚本或解释器 '{python_executable}' 或 '{run_script_path}'。")
    except subprocess.CalledProcessError as e:
        print(f"\n--- 子进程执行失败 ---")
        print(f"run.py 脚本返回了错误，退出码: {e.returncode}")
        # print("STDERR:\n", e.stderr) # 打印错误输出
        # print("STDOUT:\n", e.stdout) # 打印标准输出

    # 注意：你需要预先创建好 `basic_5layer_test.yaml` 配置文件
    # 检查配置文件是否存在
    full_config_path = os.path.join(project_root, config_file)
    if not os.path.exists(full_config_path):
        print(f"\n警告: 配置文件 '{full_config_path}' 不存在。")
        print("请确保您已经按照之前的说明创建了 `simulation/config/basic_5layer_test.yaml` 文件。")


if __name__ == "__main__":
    run_command_injection_test()